...
describe('mapDocToForm', () => {
  it('maps flat string fields and slices times to HH:mm', () => {
    ...
    expect(form.classGroup).toBe('ULEC-1A');
    expect(form.startTime).toBe('09:00');
    expect(form.endTime).toBe('10:30');
    expect(form.lessonNamesRaw).toBe('L1\nL2');
  });
  ...
  it('extracts child-table rows via the label fieldname fallbacks', () => {
    ...
    expect(form.lessonNamesRaw).toBe(
      'Part 1 Lesson 1\nPart 1 Lesson 2\nPart 1 Lesson 3',
    );
    expect(form.activitiesRaw).toBe('Listening\nReading');
  });
  ...
  it('missing fields stay empty rather than "undefined"', () => {
    ...
    expect(form.lessonNamesRaw).toBe('');
    expect(form.activitiesRaw).toBe('');
  ...
  it('non-string child rows and empty labels are skipped', () => {
    ...
    expect(form.lessonNamesRaw).toBe('plain-string\n42');
  });
});
